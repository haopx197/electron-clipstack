...
| `system:open-accessibility-settings`     | renderer → main (invoke)     | `()` → helper raises the native macOS AX-grant modal                   |
...
Build: `npm run build:helper:universal` runs `swiftc` twice (arm64 + x86_64), `lipo` merges, then [`scripts/wrap-helper-app.sh`](scripts/wrap-helper-app.sh) wraps the binary into `resources/ClipStackHelper.app` — a proper macOS bundle with `CFBundleIdentifier=com.clipstack.helper` and the ClipStack icon copied from `build/icon.icns`. A second small Swift binary, `DisclaimSpawner` (see [`native/DisclaimSpawner.swift`](native/DisclaimSpawner.swift)), is compiled to `resources/DisclaimSpawner`. Wrapping serves two goals: (a) the entry in System Settings → Accessibility shows the ClipStack icon instead of the generic Terminal placeholder raw executables get, and (b) TCC keys the grant on the stable bundle identifier so the grant survives rebuilds even when the Mach-O hash changes.

**TCC responsibility disclaimer**: Node's `child_process.spawn` uses `posix_spawn`, and macOS TCC attributes a child's Accessibility requests to the *responsible process* — the parent that spawned it. Left alone, this means the helper's AX request lands in the pane as `com.clipstack.app` (the parent) instead of `com.clipstack.helper` (the actual caller), and users granting that entry find that paste still doesn't work because the helper's own `AXIsProcessTrusted()` check keys off its own bundle. `DisclaimSpawner` is a Chrome-style shim that sits between Node and the helper: Node spawns the shim, the shim calls `posix_spawn` with `responsibility_spawnattrs_setdisclaim(&attrs, 1)` (private API) to launch the helper, and stdio inherits straight through so IPC still works. With disclaim set, TCC treats the helper as its own responsible process and grants route to the correct bundle.
...
**Info.plist keys**: the helper declares `LSUIElement=true` (hide from Dock / Cmd+Tab) but deliberately NOT `LSBackgroundOnly=true`. Both keys hide from the Dock, but `LSBackgroundOnly` signals "pure daemon, skip full registration" — Launch Services then doesn't index the bundle, `mdls` returns null for `CFBundleIdentifier`, and the AX pane can't find the icon file. Using `LSUIElement` alone hides from the Dock while still allowing full LS registration, so the ClipStack icon renders correctly.
...
**Paste-when-untrusted path**: the helper's `paste` command checks `AXIsProcessTrusted()` first and returns `paste:untrusted` without calling CGEvent when AX is missing. Skipping the CGEvent call is deliberate — a `CGEvent.post` from an untrusted process makes macOS raise the "wants to control your computer" modal on every click, which is intrusive when the banner already surfaces the same information. The one sanctioned path to grant is the banner's Open Settings button (which asks the helper to raise the same modal on demand via the `prompt-ax` command).
...
- **`codesign --deep --force` needs `--entitlements`** — running it without the JIT/unsigned-executable-memory entitlements strips them from Electron, V8 refuses to start, and the app dies at load with `task_name_for_pid: (os/kern) failure (5)`. WITH `--entitlements build/entitlements.mac.plist` it's safe. This is what [`scripts/afterPack.js`](scripts/afterPack.js) does at the end of every packaged build so the added `extraFiles` (helper.app, DisclaimSpawner) and the pre-signed Electron frameworks all end up under a single valid ad-hoc signature (`codesign --verify --deep --strict` passes). `spctl` still rejects because ad-hoc isn't a real Developer ID identity; that's disarmed by `xattr -cr` in `install.sh`.
